...
SWEP.Purpose 				= "[PH] Purpose"
SWEP.Instructions 			= "[PH] Instructions"
...
SWEP.PrintName				= "sfw_knightrifle"
SWEP.Slot					= 2
SWEP.SlotPos				= 1
...
SWEP.SciFiWorldStats = {
	...
	ReloadSpeed = 2.8,
	Recoil = 6,
	PerkSet = { SCIFI_PERK_ELEMENTAL_GRAVRIFLE_ONKILL, SCIFI_PERK_ELEMENTAL_GRAVRIFLE_OVERCHARGE, SCIFI_PERK_AMMO_RADIATION, { PerkTitle = "Game Breaking", PerkDescription = "It's fucking busted...", PerkPanelHeight = 3, PerkColor = Color( 255, 20, 20 ) } }
}
...
function SWEP:PrimaryAttack()
	...
	phys:ApplyForceCenter( pOwnerAV * 32000 + ( self:GetProjectileSpreadVector() + self.Owner:GetUp() * self:GetSciFiACC() * 0.06 ) * ( 1 + self:GetSciFiACC() * 7 ) )
	
	if ( self.Owner:IsPlayer() ) then
		self.Owner:ViewPunch( Angle( math.random( -100, -120 ), math.random( -10, 10 ), math.random( 0, 4 ) ) * ( 0.001 + self:GetSciFiACC() * 0.0005 ) )
		self:AddSciFiACC( 6 )
