...
Spell.SpriteColor = Color(255, 255, 0)          -- gunes temasi icin sari
Spell.DoSparks = true
Spell.NodeOffset = Vector(1050, -450, 0)        -- diger buyulerle cakismasin
...
function Spell:OnFire(wand)
	...
	sensor.BeamType = 3                          -- 3 = Solar Beam (orijinal SWEP kodundaki degere gore)
	sensor.Owner = owner
	sensor.Caller = owner

	local sensorPos = sensor:GetPos()

	local function CallSolarEffects()
		...
		util_Effect("pc_solar_beam", effectdata)
		util_Effect("pc_solar_beam_particle", effectdata)

	end

	CallSolarEffects()
	timer.Simple(0.05, CallSolarEffects)
	timer.Simple(0.15, CallSolarEffects)
	...
	local timerName = "mwr_solarbeam_" .. sensor:EntIndex()
	local elapsed = 0
...
HpwRewrite:AddSpell("Solar Beam", Spell)